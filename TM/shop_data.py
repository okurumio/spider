...
class AllNumber:
    ...
    def get_number(self, key):
        headers = {
            'referer': 'http://list.tmall.com/search_product.htm?q={}&type=p&spm=a220m.6910245.a2227oh.d100&from=mallfp..m_1_searchbutton&sort=d'.format(quote(key['keyword'], encoding="gbk")),
        ...
        url = 'http://list.tmall.com/m/search_items.htm?page_size=20&page_no=1&q={}&type=p&sort=d'.format(quote(key['keyword'], encoding="gbk"))
        response = requests.get(url, headers=headers)
        if response.text is None:
            print("解除验证码更换cookie")
        data = json.loads(response.text)
        key['count'] = data['total_results']
        key['page'] = data['total_page']
        del key['_id']
        return key
