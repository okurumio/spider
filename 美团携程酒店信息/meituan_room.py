...
def spider(data_url, taken, hotelId, start_day, end_date):
    ...
    while i < 5:
        try:
            response = requests.get(url=data_url, headers=headers1, proxies={'http': conn.randomOneIp('proxy:new_ip_list')}, timeout=3)
            pattern = re.compile('window.__INITIAL_STATE__=(.*);')
            a = pattern.findall(response)
            if a != []:
                break
...
if __name__ == '__main__':
    ...
    dayTime = int(time.mktime(today.timetuple()))  # 今日0时间戳
    for i in range(1, 2):
        today = datetime.date.today()
